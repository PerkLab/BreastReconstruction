...
class BreastReconstructionWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.curvedCheckedBox.setCheckState(False)
...
class BreastReconstructionLogic(ScriptedLoadableModuleLogic):
    ...
    def cropWithPlane(self, modelNode, fidList, LeftBreast, volume, surfaceArea, output):
        ...
        pModel.GetDisplayNode().BackfaceCullingOff()
            #need to compute the transform between the plane and the elipsoide


        ##################################################################################
        #Computing rotation between elipsoide and plane by computing the rotation between the two normals
        #Note: The elipsode is parellel to the x y axis, so it's normal vector is the z axis
        mathvtk = vtk.vtkMath()
        norm1 = [0, 0, 1]
...
#     clippedInputWithLoop.SetClipFunction(implictEllipsoide)  # should be loop
